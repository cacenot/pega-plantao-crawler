"""Crawler para capturar services/shifts da API."""
...
from models.service import Service, ServiceRaw
...
async def fetch_all_services(
    ...
    Args:
        ...
        delay: Delay entre páginas em segundos (default: 1.0).
        max_retries: Número máximo de tentativas em caso de erro (default: 3).
        retry_delay: Delay entre retries em segundos (default: 2.0).

    Returns:
        Lista de Service validados.
    """
    ...
    while True:
        ...
        # Extrai a lista de services do response (API retorna dict com chave 'Services')
        ...
        print(f"✅ Página {page}: {len(services)} services (total: {len(all_services)})")
...
def save_services_to_json(services: list[Service], output_dir: str) -> Path:
    ...
    Args:
        services: Lista de services validados.
